states <- c(`1`='DMN+',`2`='DMN-', `3`='S3', `4`='S4', `5`='S5')
...
d.struct <- read.csv('./../../../derivatives/WMH/cSVD_all.csv', header = TRUE) %>% 
  as_tibble() %>% 
...
filenames <- list.files(path = './../../derivatives/data/slow', pattern = 'dFC.*~.dat$', full.names = TRUE)
d.predFC <- map_df(.x = filenames, .f = ~read_csv(.), .id = 'filename') %>% 
...
d.dFC <- d.predFC %>% 
  ...
  mutate(state = dplyr::recode(as.numeric(state), !!!states) %>% factor(levels = states, ordered = TRUE)) %>% 
...
d.clinical <- read.csv('./../../../input/hchs_batch2.csv', header = TRUE)  %>% 
